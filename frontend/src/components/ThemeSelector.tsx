                ...
                >
                    {THEMES.map((t) => (
                        <li
                            ...
                            tabIndex={0}
                            onClick={() => { setTheme(t.name); close(); }}
                            onKeyDown={(e) => {
                                if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setTheme(t.name); close(); }
                            }}
                            className={`flex cursor-pointer items-center gap-2.5 px-3 py-2 text-sm transition hover:bg-white/10 focus-visible:outline-none focus-visible:bg-white/10 ${t.name === theme ? 'text-white' : 'text-white/70'
                                }`}
                        >
                            <span
