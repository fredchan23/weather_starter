...
function mergeWeatherSnapshot(
  ...
  const shouldKeepPreviousForecast =
    next.condition === CONDITION_UNAVAILABLE &&
    previous.condition !== CONDITION_NOT_REFRESHED;

  return {
    condition:
      shouldKeepPreviousForecast && previous.condition
        ? previous.condition
        : next.condition,
    observed_at: next.observed_at || previous.observed_at || '',
    source:
      (shouldKeepPreviousForecast ? previous.source : null) ?? next.source,
    ...
    wind_direction_degrees:
      next.wind_direction_degrees ?? previous.wind_direction_degrees,
    ...
    psi_twenty_four_hourly:
      next.psi_twenty_four_hourly ?? previous.psi_twenty_four_hourly,
    ...
    forecast_periods:
      next.forecast_periods.length > 0
        ? next.forecast_periods
        : previous.forecast_periods,
    daily_forecast:
      next.daily_forecast.length > 0
        ? next.daily_forecast
